...
for file in video/*; do
    ...
    if [ -f "$FILE" ]; then
        ...
        #Lag GIF
        ffmpeg -n -i "temp/$(basename "${file}")_map/output_$(basename "${file%%.*}").mp4" -vf "setpts=PTS/20,fps=30,scale=320:-1:flags=lanczos,split[s0][s1];[s0]palettegen[p];[s1][p]paletteuse" -loop 0 "gif/$(basename "${file%%.*}").gif"
        #Rydd
